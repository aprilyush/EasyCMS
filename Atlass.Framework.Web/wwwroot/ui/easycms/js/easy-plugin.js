...
; (function ($) {
    "use strict";
    $.fn.easyRadioBox = function (option, param) {
        if (typeof option == 'string') {
            return $.fn.easyRadioBox.methods[option](this, param);
        }
        var target = $(this);
        ...
        var dom = {
            ...
            render: function (setting) {
                ...
                if (setting.default != undefined) {
                    target.easyRadioBox("setValue", setting.default);
                }
            }
        };
        dom.loadData();
    ...
    $.fn.easyCheckBox = function (option, param) {
        if (typeof option == 'string') {
            return $.fn.easyCheckBox.methods[option](this, param);
        }
        var target = $(this);
        var targetId = target.attr('id');
        if (!targetId) {
        ...
        var dom = {
            ...
            render: function (setting) {
                ...
                if (setting.default != undefined) {
                    target.easyCheckBox("setValue", setting.default);
                }
            }
        };
        dom.loadData();
    ...
    // 下拉框select，和easyComboxBoxTree对应
    $.fn.easyComboBox = function (option, param) {
        if (typeof option == 'string') {
            return $.fn.easyComboBox.methods[option](this, param);
        }
        var target = $(this);
        var targetId = target.attr('id');
        if (!targetId) {
        ...
        var dom = {
            ...
            render: function (setting) {
                if (setting.data && setting.data.length >= 0) {
                    ...
                    var html = "<select id='" + id + "' name='" + id + "' class='" + easy.toString(setting.class) + " select2' " + mutiple + " maxheight=" + setting.maxHeight + ">";
                    ...
                    if (setting.default != undefined) {
                        target.easyComboBox("setValue", setting.default);
                    }
                }
            }
        };
        ...
        $(this).find("[col]").each(function (i, control) {
            ...
            if (control.tagName == "INPUT") {
                ...
                if ($(control).find("#" + id + "_tree").length > 0) {
                    data[field] = $(control).easyComboBoxTree("getValue");
                } else if ($(control).find("#" + id + "_select").length > 0) {
                    data[field] = $(control).easyComboBox("getValue");
                }
                else if ($(control).find("input[type=checkbox]").length > 0) {
                    data[field] = $(control).easyCheckBox("getValue");
                } else if ($(control).find("input[type=radio]").length > 0) {
                    data[field] = $(control).easyRadioBox("getValue");
                } else {
    ...
    $.fn.setWebControls = function (data) {
        $(this).find("[col]").each(function (i, control) {
            ...
            if (control.tagName == "INPUT") {
                ...
                if ($(control).find("#" + id + "_tree").length > 0) {
                    $(control).easyComboBoxTree("setValue", data[field]);
                } else if ($(control).find("#" + id + "_select").length > 0) {
                    $(control).easyComboBox("setValue", data[field]);
                } else if ($(control).find("input[type=checkbox]").length > 0) {
                    $(control).easyCheckBox("setValue", data[field]);
                } else if ($(control).find("input[type=radio]").length > 0) {
                    $(control).easyRadioBox("setValue", data[field]);
                } else {
