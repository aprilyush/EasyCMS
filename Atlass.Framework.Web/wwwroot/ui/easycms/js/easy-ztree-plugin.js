(function ($) {
    "use strict";
    $.fn.eTree = function (option, param) {
        if (typeof option == 'string') {
            return $.fn.eTree.methods[option](this, param);
        }
        var _option = $.extend({}, $.fn.eTree.defaults, option || {});
        var target = $(this);
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status) {
                    let treeData=res.data.ztreeData;
                    var tree = $.fn.zTree.init($("#" + id), _option,treeData);
    ...
    $.fn.eTree.methods = {
        getCheckedNodes: function (target, column) {
            ...
            var _column = jutils.empty(column) ? "id" : column;
    ...
    $.fn.eComboBoxTree = function (option, param) {
        if (typeof option == 'string') {
            return $.fn.eComboBoxTree.methods[option](this, param);
        }
        var _option = $.extend({}, $.fn.eComboBoxTree.defaults, option || {});
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status) {
                    ...
                    target.ztree = $.fn.zTree.init($("#" + eleTreeId), _option, treeData);
        ...
        $(document).click(function (e) {
            ...
            if (!$(tar).hasClass('form-control')) {
                ...
                if (jutils.empty(tarId) || tarId.indexOf("_tree") == -1) {
                    var targetTree = $("#" + eleTreeId);
                    targetTree.hide();
                    e.stopPropagation();
                }
