...
{
    public class OverTimeReplyJob:IJob
    {
        public void Excute(string wxAppId, string taskId)
        {
            try
            {
                ...
                var stime = DateTime.Now.AddHours(-6);
                ...
                var questions = Sqldb.Select<expert_question>()
                    ...
                    .OrderBy(s => s.id)
                    .Select(s => new { id = s.id,qtitle=s.qtitle ,questionNo=s.question_no,openid = s.insert_id }).ToList();
                ...
                if (count > 0)
                {
                    ...
                    try
                    {
                        ...
                        {
                            Task.Factory.StartNew(() => MessagePush.NewQuestion(wxAppId, q.openid, q.questionNo.ToString(), q.qtitle));
                        }

                        var ids = questions.Select(s => s.id).Distinct().ToList();
                        Sqldb.Transaction(() =>
                        {
                            var affrows = Sqldb.Update<expert_question>().Set(s => s.last_time, nowTime)
                           .Where(s => ids.Contains(s.id))
                           .ExecuteAffrows();
                            if (affrows < 1)
                                throw new Exception("更新超时主问题失败");
                            affrows = Sqldb.Update<expert_question>().Set(s => s.last_time, nowTime)
                                .Where(s => ids.Contains(s.pid))
                                .ExecuteAffrows();
                            if (affrows < 1)
                                throw new Exception("更新超时追问问题失败");
                        });
